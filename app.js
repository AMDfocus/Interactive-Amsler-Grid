// File: app.js
// We are using “text/babel” so this file can remain as JSX
// IMPORTANT: Because we used Babel Standalone, we must reference React and ReactDOM
//            through global variables, e.g. React.useState, React.useEffect, etc.
...
const AmslerGrid = forwardRef(({ width = "400px", height = "400px", onDistortionChange, inverse = false }, ref) => {
  ...
  const handleMouseMove = (e) => {
    if (drawing) setCurrentLine((l) => [...l, getPoint(e)]);
  };
  const handleMouseUp = () => {
    if (drawing) setLines((l) => [...l, currentLine]);
    setDrawing(false);
  };
  ...
  function integrateStroke() {
    ...
    if (dx < dy) {
      ...
      pts = Array.from({ length: divisions + 1 }, (_, i) => ({
        x: f(i * stepY),
        y: i * stepY,
      }));
    } else {
  ...
  return (
    <div className="flex flex-col items-center p-4">
      ...
      <div className="mb-4 text-center">
        <p className="mb-1">
          <strong>How to Use:</strong> Draw wavy lines on the grid where you see
          distortion.
        </p>
        <p className="mb-1">
          Click <em>Next</em> to integrate the stroke into the nearest grid
          line.
        </p>
        <p>
          Use <em>Reset</em> to clear the grid, or toggle between
          Distorted/Undistorted views.
        </p>
      </div>
      <AmslerGrid
        ref={gridRef}
      ...
      <div className="mt-2 space-x-2">
        ...
        >
          {inverseView ? "Distorted View" : "Undistorted View"}
        </button>
      </div>
